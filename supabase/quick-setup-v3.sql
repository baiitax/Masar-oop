...
-- Step 9: Create helper function to get user's organization IDs
-- This avoids self-referencing in RLS policies
...
-- Step 13: RLS Policies (NO self-referencing!)
...
-- Organizations: Anyone authenticated can create, members can view
CREATE POLICY "Organizations can be created"
...
CREATE POLICY "Organizations viewable by members"
  ...
  USING (
    deleted_at IS NULL
    AND id IN (SELECT public.get_user_organization_ids())
  );

-- Roles: Read-only for authenticated users
CREATE POLICY "Roles viewable"
  ON public.roles FOR SELECT
  USING (auth.uid() IS NOT NULL AND active = TRUE);

-- Organization Members: Users can see their own memberships
-- NO self-referencing policy to avoid recursion!
...
-- Done!
SELECT 'MASAR database setup complete! No recursion errors.' as status;
