...
import { Shield, Search, Lock, FileText, DollarSign, Ship, Eye, Activity, CheckCircle, ArrowRight, Globe, Layers, Database, Cpu, Network, Users, Truck, BarChart3, Target } from 'lucide-react';

export default function PlatformPage() {
  const s = { navy: '#0B1F3A', gold: '#C9A24A', text: '#142235', textSec: '#5B6778', border: '#E4E7EC', green: '#2D7D46' };
  ...
  const modules = [
    ...
    { icon: DollarSign, title: 'Finance Workspace', desc: 'Structured financing packages for capital partners with exposure tracking and risk assessment.', color: '#C9A24A' },
  ...
  return (
    <PublicLayout 
      title="The MASAR Platform" 
      subtitle="Twelve operational modules powering the complete transaction lifecycle — from counterparty verification to settlement."
      breadcrumb={[{ label: 'Platform' }]}
      heroImage="/images/trade-analytics.png"
    >
      {/* Intro */}
        ...
        <div style={s.container}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', alignItems: 'center' }}>
            <div>
              ...
              <Link href="/auth" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 28px', background: 'linear-gradient(135deg, #C9A24A, #E3C875)', color: '#0B1F3A', borderRadius: '12px', fontSize: '15px', fontWeight: 700, textDecoration: 'none' }}>
      ...
      <section style={{ padding: '4rem 1.5rem', background: s.navy, textAlign: 'center' }}>
        <div style={{ ...s.container, maxWidth: '600px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/auth" style={{ ...s.btnPrimary, display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 28px', background: 'linear-gradient(135deg, #C9A24A, #E3C875)', color: '#0B1F3A', borderRadius: '12px', fontSize: '15px', fontWeight: 700, textDecoration: 'none' }}>Start a Trade <ArrowRight size={16} /></Link>
            <Link href="/contact" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 28px', background: 'rgba(255,255,255,0.1)', color: 'white', borderRadius: '12px', fontSize: '15px', fontWeight: 600, textDecoration: 'none', border: '1px solid rgba(255,255,255,0.2)' }}>Contact Us</Link>
          </div>
        </div>
      </section>
    </PublicLayout>
