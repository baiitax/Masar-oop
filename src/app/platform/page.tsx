...
import { 
  ...
  Landmark, ScrollText, Database, Cpu, Server, Wifi, ShieldCheck as ShieldCheckIcon
} from 'lucide-react';
...
export default function PlatformPage() {
  ...
  const whatsappUrl = `https://wa.me/2348022220247?text=${encodeURIComponent('Hello MASAR, I would like to learn more about your platform.')}`;
  ...
  const platformModules = [
    { icon: ShieldCheck, title: 'KYB Engine', desc: 'Automated Know Your Business verification with sanctions screening, UBO mapping, and risk scoring.', color: '#3B82F6' },
    { icon: ClipboardCheck, title: 'Compliance Engine', desc: 'Lane-specific compliance automation with document management and clearance readiness scoring.', color: '#8B5CF6' },
    { icon: Eye, title: 'Inspection Engine', desc: 'Independent inspection coordination with quality scoring and chain of custody tracking.', color: '#2D7D46' },
    { icon: DollarSign, title: 'Finance Engine', desc: 'Trade finance coordination with escrow management and settlement waterfall.', color: '#C9A24A' },
    { icon: Truck, title: 'Logistics Engine', desc: 'Shipment tracking with port verification and delivery confirmation.', color: '#0B1F3A' },
    { icon: Lock, title: 'Release Engine', desc: 'Conditional release with dual approval and authorization workflows.', color: '#14B8A6' },
    { icon: Database, title: 'Audit Engine', desc: 'Immutable audit trail with hash chain for complete transaction evidence.', color: '#6366F1' },
    { icon: Cpu, title: 'Protocol Engine', desc: 'Transaction state machine with workflow automation and SLA monitoring.', color: '#EC4899' },
  ];

  const technicalSpecs = [
    { label: 'Database', value: 'Supabase PostgreSQL', desc: 'Row Level Security, 60+ tables' },
    { label: 'Authentication', value: 'Supabase Auth', desc: 'JWT, MFA, session management' },
    { label: 'API', value: 'Next.js API Routes', desc: 'REST, versioned, RBAC protected' },
    { label: 'Storage', value: 'Supabase Storage', desc: 'Private buckets, signed URLs' },
    { label: 'Realtime', value: 'Supabase Realtime', desc: 'Selective subscriptions' },
    { label: 'Hosting', value: 'Vercel', desc: 'Edge functions, CDN, SSL' },
  ];

  const securityFeatures = [
    { title: 'Row Level Security', desc: 'Database-level tenant isolation ensuring organizations can only access their own data.' },
    { title: 'Role-Based Access Control', desc: '17 roles with 30+ granular permissions for fine-grained authorization.' },
    { title: 'API Gateway', desc: 'All requests authenticated, validated, rate-limited, and logged.' },
    { title: 'Encrypted Storage', desc: 'All documents stored in private buckets with time-limited signed URLs.' },
    { title: 'Audit Trail', desc: 'Immutable event log with hash chain for complete traceability.' },
    { title: 'Input Validation', desc: 'Zod schemas validate all inputs before processing.' },
  ];
    ...
    <div style={{ minHeight: '100vh', fontFamily: "'Inter', 'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
          ...
          <div style={{ ...(scrolled ? { ...s.glassLight, borderRadius: '16px', padding: '10px 20px' } : { padding: '0' }), transition: 'all 0.4s ease' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div className="hidden lg:flex" style={{ alignItems: 'center', gap: '20px' }}>
                <Link href="/platform" style={{ fontSize: '13px', fontWeight: 600, color: '#C9A24A', textDecoration: 'none' }}>Platform</Link>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'white', padding: '2rem', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {['Platform', 'How It Works', 'For Buyers', 'For Exporters', 'Trust & Compliance', 'About', 'Contact'].map((item) => (
              <Link key={item} href={`/${item.toLowerCase().replace(/\s+/g, '-').replace('&-', '')}`} onClick={() => setMobileMenuOpen(false)} style={{ fontSize: '18px', fontWeight: item === 'Platform' ? 700 : 500, color: item === 'Platform' ? '#C9A24A' : '#0B1F3A', textDecoration: 'none', padding: '12px 0', borderBottom: '1px solid #E5E9F0' }}>{item}</Link>
            ))}
      ...
      <section style={{ position: 'relative', minHeight: '70vh', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ ...s.container, padding: '8rem 1.5rem 4rem', position: 'relative', zIndex: 10, textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 16px', background: 'rgba(201,162,74,0.1)', border: '1px solid rgba(201,162,74,0.2)', borderRadius: '6px', marginBottom: '2rem' }}>
            <Cpu size={14} color="#C9A24A" />
            <span style={{ fontSize: '11px', fontWeight: 600, color: '#C9A24A', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Platform</span>
          </div>
          <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, color: 'white', lineHeight: 1.15, marginBottom: '1.5rem', maxWidth: '700px', margin: '0 auto 1.5rem' }}>
            The <span style={{ color: '#C9A24A' }}>Protocol Engine</span> Behind Cross-Border Trade
          </h1>
          <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.65)', lineHeight: 1.7, maxWidth: '600px', margin: '0 auto 2.5rem' }}>
            MASAR is not a marketplace. It is the trust, verification, compliance and workflow infrastructure that makes cross-border commerce executable.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', justifyContent: 'center' }}>
            {['KYB Engine', 'Compliance Engine', 'Inspection Engine', 'Finance Engine', 'Release Engine', 'Audit Trail'].map((item, idx) => (
      ...
      <section style={{ ...s.section, background: 'white' }}>
        <div style={s.container}>
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <h2 style={s.sectionTitle}>Protocol Engines</h2>
            <p style={{ ...s.sectionSubtitle, margin: '0 auto' }}>Modular engines that orchestrate every aspect of cross-border trade.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
            {platformModules.map((module, idx) => (
              <div key={idx} style={{ ...s.card, padding: '24px', borderTop: `3px solid ${module.color}` }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: `${module.color}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
                  <module.icon size={24} color={module.color} />
                </div>
                <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#0B1F3A', marginBottom: '8px' }}>{module.title}</h3>
                <p style={{ fontSize: '13px', color: '#5B6778', margin: 0, lineHeight: 1.6 }}>{module.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ ...s.section, background: '#F7F9FC' }}>
        <div style={s.container}>
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            <div style={{ ...s.goldDivider, margin: '0 auto 1.5rem' }} />
            <h2 style={s.sectionTitle}>Technical Architecture</h2>
            <p style={{ ...s.sectionSubtitle, margin: '0 auto' }}>Built on modern, scalable, secure infrastructure.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
            {technicalSpecs.map((spec, idx) => (
              <div key={idx} style={{ ...s.card, padding: '20px', textAlign: 'center' }}>
                <p style={{ fontSize: '11px', color: '#9BA3AE', margin: '0 0 4px 0', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{spec.label}</p>
                <p style={{ fontSize: '16px', fontWeight: 700, color: '#0B1F3A', margin: '0 0 4px 0' }}>{spec.value}</p>
                <p style={{ fontSize: '11px', color: '#5B6778', margin: 0 }}>{spec.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ ...s.section, background: 'white' }}>
        <div style={s.container}>
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            <div style={{ ...s.goldDivider, margin: '0 auto 1.5rem' }} />
            <h2 style={s.sectionTitle}>Security by Design</h2>
            <p style={{ ...s.sectionSubtitle, margin: '0 auto' }}>Defense-in-depth security at every layer of the stack.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '20px' }}>
            {securityFeatures.map((feature, idx) => (
              <div key={idx} style={{ ...s.card, padding: '24px', display: 'flex', gap: '14px' }}>
                <ShieldCheck size={20} color="#2D7D46" style={{ flexShrink: 0, marginTop: '2px' }} />
                <div>
                  <h4 style={{ fontSize: '14px', fontWeight: 700, color: '#0B1F3A', margin: '0 0 4px 0' }}>{feature.title}</h4>
      ...
      <section style={{ ...s.section, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ ...s.container, position: 'relative', zIndex: 10, textAlign: 'center', maxWidth: '800px' }}>
          <h2 style={{ fontSize: 'clamp(1.75rem, 3vw, 2.5rem)', fontWeight: 800, color: 'white', marginBottom: '1rem' }}>Experience the Protocol</h2>
          <p style={{ fontSize: '1.05rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, marginBottom: '2.5rem' }}>Join MASAR and trade through infrastructure built for trust, verification and accountability.</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px', justifyContent: 'center' }}>
            <Link href="/register" style={s.btnPrimary}>Get Started <ArrowRight size={16} /></Link>
            <Link href="/how-it-works" style={s.btnSecondary}>How It Works</Link>
          </div>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
            ...
            <div>
              ...
              {['Platform', 'How It Works', 'For Buyers', 'For Exporters', 'Capital Partners', 'Trust & Compliance', 'About'].map((link) => (
                <Link key={link} href={`/${link.toLowerCase().replace(/\s+/g, '-').replace('&-', '')}`} style={{ display: 'block', fontSize: '12px', color: link === 'Platform' ? '#C9A24A' : 'rgba(255,255,255,0.4)', textDecoration: 'none', marginBottom: '6px' }}>{link}</Link>
              ))}
            </div>
            <div>
